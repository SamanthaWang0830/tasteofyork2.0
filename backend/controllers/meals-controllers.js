...
//crypto allows you to hash plain texts before storing them in the db.
...
const deleteFile=(fileName)=> {
    ...
    return s3.deleteObject(deleteParams, (err)=>{
        console.log("something wrong in this deleteFile function")
    })
}
...
const createMeal=async(req,res,next)=>{
  //已经出现的错误 make sure that you validate the input and report any errors before creating in the db.
...
const deleteMealById=async(req,res,next)=>{
  ...
  //fs.unlink(imagePath,err=>console.log("this is the error occurred in the delete in aws",err))
