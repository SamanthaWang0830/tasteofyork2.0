...
const MealCard: React.FC<MealCardProps>=({pick, dislikeHandler,likeHandler, deleteHandler})=>{
    const { name, description, image, likeCount, dislikeCount,_id,creator} = pick;
    ...
    const fetchMealOriginalInfoHandler=async()=>{
        ...
        try {
            const responseData=await sendRequest(import.meta.env.VITE_BACKEND_URL+`/meals/${_id}`)
